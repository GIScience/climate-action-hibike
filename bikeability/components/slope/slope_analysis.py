...
def build_slope_artifact(
    ...
    legend_lower_bound, legend_upper_bound = 0, 6
    path_slopes_data['color'] = get_continuous_colors(
        path_slopes_data['slope'],
        cmap_name,
        vmin=legend_lower_bound,
        vmax=legend_upper_bound,
    )
    ...
    legend = Legend(
        legend_data=ContinuousLegendData(
            ...
            ticks={
                ...
                'Very Gentle (1.5%)': 0.25,
                'Moderate (3%)': 0.5,
                'Considerable (4.5%)': 0.75,
...
def summarise_slope(
    ...
    lower_bound, upper_bound = 0, 7  # 6%+1
    ...
    # Replace the last tick label with '>100'
    tick_vals = list(bin_edges[::3])  # every 3rd edge to avoid crowding
    tick_text = [str(round(v)) for v in tick_vals]
    tick_text[-1] = f'≥{tick_text[-1]}'  # if int(tick_text[-1]) < upper_bound else '≥6'
